fn main() {
    ...
    // This means the slice starts from index 2 and goes up to the end
    println!("index 2 to end sliced - {:?}", slice3);

    // omit the start index and the end index
    // reference the whole array
    let slice4 = &numbers[..];
    // This means the slice starts from index 0 and goes up to the last index (exclusive)
    println!("index 0 to end sliced - {:?}", slice4);
}
